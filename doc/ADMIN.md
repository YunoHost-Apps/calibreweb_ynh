...
Warning : Kobo devices are not compatible with hardening ciphers for NGINX. Do not activate "modern" policy as described [here](https://doc.yunohost.org/en/admin/security/?persistLocale=true#hardening-ciphers-used-by-nginx-ssh-dovecot-postfix). If you use "modern" policy, Kobo devices will not be able to connect to your server with no explanation other than "Internet not avalaible".
